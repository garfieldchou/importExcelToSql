...
namespace SteamData.HardwareSoftwareSurvey {
  public class HardwareSoftwareSurveyUtils : ExcelContent {
    ...
    public static void ImportProceUsageDetail (SteamDataContext db) {
      var usageDetails = content.Tables[3];
      int monthStart = usageDetails.Rows[0][2].ToString ().MonthStringToInt ();
      int startYear = reportDate.Year;
      string category = usageDetails.Rows[0][1].ToString ();
      int latestYearInDb = int.MinValue;
      ...
      var detailInDb = (
        from d in db.ProceUsageDetails orderby d.Year descending, d.Month descending select d).FirstOrDefault ();
      ...
      for (int i = 1; i < usageDetails.Rows.Count; i++) {
        ...
        for (int col = 2; col < 7; col++) {
          ...
          db.ProceUsageDetails.Add (new ProceUsageDetail {
            Year = year,
              Month = month,
              Category = category,
              Item = item,
    ...
    public static void ImportPcPhyCpuDetail (SteamDataContext db) {
      var usageDetails = content.Tables[4];
      int monthStart = usageDetails.Rows[0][2].ToString ().MonthStringToInt ();
      int startYear = reportDate.Year;
      string category = usageDetails.Rows[0][1].ToString ();
      int latestYearInDb = int.MinValue;
      ...
      var detailInDb = (
        from d in db.PcPhyCpuDetails orderby d.Year descending, d.Month descending select d).FirstOrDefault ();
      ...
      for (int i = 1; i < usageDetails.Rows.Count; i++) {
        ...
        for (int col = 2; col < 7; col++) {
          ...
          db.PcPhyCpuDetails.Add (new PcPhyCpuDetail {
            Year = year,
              Month = month,
              Category = category,
              Item = item,
